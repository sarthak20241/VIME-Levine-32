...
# Necessary packages
import numpy as np
from sklearn.metrics import accuracy_score, roc_auc_score, f1_score, silhouette_score, adjusted_rand_score, davies_bouldin_score, calinski_harabasz_score
...
def perf_metric(metric, y_test, y_test_hat):
    ...
    if metric == 'acc':
        ...
        result = f1_score(np.argmax(y_test, axis=1),
                          np.argmax(y_test_hat, axis=1), average='weighted')
        ...
        result = recall_score(
            np.argmax(y_test, axis=1), np.argmax(y_test_hat, axis=1), average='weighted')
